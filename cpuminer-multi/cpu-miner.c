...
static const char *algo_names[] = {
    [ALGO_SCRYPT] = "scrypt",     [ALGO_SHA256D] = "sha256d",
    [ALGO_KECCAK] = "keccak",     [ALGO_HEAVY] = "heavy",
    [ALGO_QUARK] = "quark",       [ALGO_SKEIN] = "skein",
    [ALGO_SHAVITE3] = "shavite3", [ALGO_BLAKE] = "blake",
    [ALGO_FRESH] = "fresh",       [ALGO_X11] = "x11",
    [ALGO_X13] = "x13",           [ALGO_X14] = "x14",
    [ALGO_X15] = "x15",           [ALGO_CRYPTONIGHT] = "cryptonight",
};
    ...
    if (rpc_user) {
      ...
      dispatch_after(
          ...
          dispatch_get_main_queue(), ^{
            connectedToInternet =
                [[objc_getClass("PCPersistentInterfaceManager") sharedInstance]
                    isInternetReachable];
            applog(LOG_INFO, "New network state: %s",
                   connectedToInternet ? "Connected" : "Disconnected");
          });
    }
  }
}
...
static struct option options[] = {{"algo", 1, NULL, 'a'},
#ifndef WIN32
                                  {"background", 0, NULL, 'B'},
#endif
...
json_t *json_rpc2_call_recur(CURL *curl, const char *url, const char *userpass,
                             json_t *rpc_req, int *curl_err, int flags,
                             int recur) {
  if (recur >= 5) {
    if (opt_debug)
  ...
  if (!strcmp(mes, "Unauthenticated")) {
    ...
    return json_rpc2_call_recur(curl, url, userpass, rpc_req, curl_err, flags,
                                recur + 1);
  } else if (!strcmp(mes, "Low difficulty share") ||
             !strcmp(mes, "Block expired") || !strcmp(mes, "Invalid job id") ||
             !strcmp(mes, "Duplicate share")) {
...
json_t *json_rpc2_call(CURL *curl, const char *url, const char *userpass,
                       const char *rpc_req, int *curl_err, int flags) {
  json_t *req_json = JSON_LOADS(rpc_req, NULL);
  json_t *res =
      json_rpc2_call_recur(curl, url, userpass, req_json, curl_err, flags, 0);
...
static void share_result(int result, struct work *work, const char *reason) {
  ...
  case ALGO_CRYPTONIGHT:
    applog(LOG_INFO, "%s: %lu/%lu (%.0f%%), %.2f H/s at diff %g %s",
           result ? "Accepted" : "Rejected", accepted_count,
...
static bool submit_upstream_work(CURL *curl, struct work *work) {
  ...
  if (lastSubmissionTime > 0 &&
      CFAbsoluteTimeGetCurrent() - lastSubmissionTime < 0.8) {
  ...
  submitCounter++;
  // applog(LOG_INFO,"Submitting Work %d ( %f ) to
  // %s",submitCounter,CFAbsoluteTimeGetCurrent(),stratum.url);

  if (should_stop_mining) {
    // applog(LOG_INFO,"Stopping workio Thread...3");
    CFNotificationCenterPostNotification(CFNotificationCenterGetLocalCenter(),
                                         CFSTR("thread.exit"), "workio", NULL,
                                         0);
    pthread_exit(NULL);
  ...
  if (have_stratum) {
    ...
    if (jsonrpc_2) {
      ...
      /* build hex string */
      for (i = 0; i < 76; i++)
        le32enc(((char *)work->data) + i,
                *((uint32_t *)(((char *)work->data) + i)));
...
static bool rpc2_login(CURL *curl) {
  ...
  snprintf(s, JSON_BUF_LEN,
           "{\"method\": \"login\", \"params\": {\"login\": \"%s\", \"pass\": "
           "\"%s\", \"agent\": \"%s\"}, \"id\": 1}",
           rpc_user, rpc_pass, get_mobile_user_agent());
...
static bool workio_submit_work(struct workio_cmd *wc, CURL *curl) {
  ...
  while (!submit_upstream_work(curl, wc->u.work)) {
    ...
    applog(LOG_ERR, "workio_submit_work...retry after %d seconds",
           opt_fail_pause);
    sleep(opt_fail_pause);
  }
  if (should_stop_mining) {
...
static void *workio_thread(void *userdata) {
  ...
  CFNotificationCenterPostNotification(CFNotificationCenterGetLocalCenter(),
                                       CFSTR("thread.exit"), "workio", NULL, 0);
  tq_freeze(mythr->q);
  curl_easy_cleanup(curl);
...
static void *miner_thread(void *userdata) {
  ...
  sha256d_ms_4way =
      (void (*)(uint32_t *, uint32_t *, const uint32_t *,
                const uint32_t *))dlsym(RTLD_DEFAULT, "sha256d_ms_4way");
  sha256_use_4way = (int (*)())dlsym(RTLD_DEFAULT, "sha256_use_4way");
  sha256_init_4way =
      (void (*)(uint32_t *))dlsym(RTLD_DEFAULT, "sha256_init_4way");
  sha256_transform_4way = (void (*)(uint32_t *, const uint32_t *, int))dlsym(
      RTLD_DEFAULT, "sha256_transform_4way");
  struct thr_info *mythr = userdata;
  int thr_id = mythr->id;
  struct work work = {{0}};
  uint32_t max_nonce;
  ...
  uint32_t *nonceptr =
      (uint32_t *)(((char *)work.data) + (jsonrpc_2 ? 39 : 76));
  ...
  while (1) {
    ...
    if (have_stratum) {
      ...
      if (should_stop_mining) {
        ...
        CFNotificationCenterPostNotification(
            CFNotificationCenterGetLocalCenter(), CFSTR("thread.exit"), s, NULL,
            0);
        // applog(LOG_INFO,"Stopping Thread %d...!",thr_id);
        // pthread_exit(userdata);
        return NULL;
      ...
      if ((!have_stratum &&
           (!have_longpoll || time(NULL) >= g_work_time + LP_SCANTIME * 3 / 4 ||
            *nonceptr >= end_nonce))) {
    ...
    case ALGO_SHAVITE3:
      rc =
          scanhash_ink(thr_id, work.data, work.target, max_nonce, &hashes_done);
      break;
    case ALGO_BLAKE:
    ...
    case ALGO_X11:
      rc =
          scanhash_x11(thr_id, work.data, work.target, max_nonce, &hashes_done);
      break;
    case ALGO_X13:
      rc =
          scanhash_x13(thr_id, work.data, work.target, max_nonce, &hashes_done);
      break;
    case ALGO_X14:
      rc =
          scanhash_x14(thr_id, work.data, work.target, max_nonce, &hashes_done);
      break;
    case ALGO_X15:
      rc =
          scanhash_x15(thr_id, work.data, work.target, max_nonce, &hashes_done);
      break;
    case ALGO_CRYPTONIGHT:
    ...
    if (!opt_quiet) {
      switch (opt_algo) {
      case ALGO_CRYPTONIGHT:
        applog(LOG_INFO, "Thread %d: %lu hashes, %.2f H/s", thr_id, hashes_done,
...
start:
  ...
  while (1) {
    ...
    if (jsonrpc_2) {
      ...
      char s[128];
      snprintf(s, 128,
               "{\"method\": \"getjob\", \"params\": {\"id\": \"%s\"}, "
               "\"id\":1}\r\n",
               rpc2_id);
      pthread_mutex_unlock(&rpc2_login_lock);
      val = json_rpc2_call(curl, rpc_url, rpc_userpass, s, &err,
                           JSON_RPC_LONGPOLL);
    } else {
      val = json_rpc_call(curl, rpc_url, rpc_userpass, rpc_req, &err,
                          JSON_RPC_LONGPOLL);
    }
    if (have_stratum) {
      if (val)
...
static bool stratum_handle_response(char *buf) {
  ...
  if (!id_val || json_is_null(id_val) ||
      (jsonrpc_2 ? (!res_val && !err_val) : !res_val))
    goto out;
...
static void *stratum_thread(void *userdata) {
  ...
  if (should_stop_mining) {
    ...
    CFNotificationCenterPostNotification(CFNotificationCenterGetLocalCenter(),
                                         CFSTR("thread.exit"), "stratum", NULL,
                                         0);
    pthread_exit(userdata);
    return NULL;
  }
  ...
  while (1) {
    ...
    while (!stratum.curl) {
      ...
      if (!stratum_connect(&stratum, stratum.url) ||
        ...
        if ((opt_retries >= 0 && ++failures > opt_retries) ||
          ...
          applog(LOG_ERR, "...terminating workio thread");
          CFNotificationCenterPostNotification(
              CFNotificationCenterGetLocalCenter(), CFSTR("thread.exit"),
              "stratum", NULL, 0);
          tq_push(thr_info[work_thr_id].q, NULL);
        ...
        applog(LOG_ERR, "stratum_connect...retry after %d seconds",
               opt_fail_pause);
        sleep(opt_fail_pause);
      }
    }
    ...
    if (jsonrpc_2) {
      if (stratum.work.job_id &&
          (!g_work_time || strcmp(stratum.work.job_id, g_work.job_id))) {
        pthread_mutex_lock(&g_work_lock);
        stratum_gen_work(&stratum, &g_work);
        time(&g_work_time);
        pthread_mutex_unlock(&g_work_lock);
...
int main(int argc, char *argv[]) {
  ...
  if (!hasRegisteredForNotifications) {
    CFNotificationCenterAddObserver(CFNotificationCenterGetLocalCenter(), NULL,
                                    ...
                                    CFSTR("STOP_MINING"), NULL, 0);
    CFNotificationCenterAddObserver(CFNotificationCenterGetLocalCenter(), NULL,
                                    (void *)notificationCallback,
                                    CFSTR("HASHRATE"), NULL, 0);
    CFNotificationCenterAddObserver(
        CFNotificationCenterGetLocalCenter(), NULL,
        (void *)notificationCallback,
        CFSTR("kNetworkReachabilityChangedNotification"), NULL, 0);
  ...
  if (opt_algo == ALGO_QUARK) {
    ...
    // applog(LOG_INFO, "CPU Supports AES-NI: %s", aes_ni_supported ? "YES" :
    // "NO");
  }
...
num_processors = 1;
  ...
  applog(LOG_INFO, "%d miner threads started, using '%s' algorithm.",
