...
#define state_out(y, x)                                                        \
  ...
  so(y, x, 3)
#define round(y, x, k)                                                         \
  y[0] = (k)[0] ^ (t_fn[0][x[0] & 0xff] ^ t_fn[1][(x[1] >> 8) & 0xff] ^        \
                   t_fn[2][(x[2] >> 16) & 0xff] ^ t_fn[3][x[3] >> 24]);        \
  y[1] = (k)[1] ^ (t_fn[0][x[1] & 0xff] ^ t_fn[1][(x[2] >> 8) & 0xff] ^        \
                   t_fn[2][(x[3] >> 16) & 0xff] ^ t_fn[3][x[0] >> 24]);        \
  y[2] = (k)[2] ^ (t_fn[0][x[2] & 0xff] ^ t_fn[1][(x[3] >> 8) & 0xff] ^        \
                   t_fn[2][(x[0] >> 16) & 0xff] ^ t_fn[3][x[1] >> 24]);        \
  y[3] = (k)[3] ^ (t_fn[0][x[3] & 0xff] ^ t_fn[1][(x[0] >> 8) & 0xff] ^        \
                   t_fn[2][(x[1] >> 16) & 0xff] ^ t_fn[3][x[2] >> 24]);
#define to_byte(x) ((x)&0xff)
#define bval(x, n) to_byte((x) >> (8 * (n)))

#define fwd_var(x, r, c)                                                       \
  (r == 0                                                                      \
       ? (c == 0 ? s(x, 0) : c == 1 ? s(x, 1) : c == 2 ? s(x, 2) : s(x, 3))    \
       : r == 1                                                                \
             ? (c == 0 ? s(x, 1)                                               \
                       : c == 1 ? s(x, 2) : c == 2 ? s(x, 3) : s(x, 0))        \
             : r == 2                                                          \
                   ? (c == 0 ? s(x, 2)                                         \
                             : c == 1 ? s(x, 3) : c == 2 ? s(x, 0) : s(x, 1))  \
                   : (c == 0 ? s(x, 3)                                         \
                             : c == 1 ? s(x, 0) : c == 2 ? s(x, 1) : s(x, 2)))
...
#define d_4(t, n, b, e, f, g, h)                                               \
  ALIGN const t n[4][256] = {b(e), b(f), b(g), b(h)}
