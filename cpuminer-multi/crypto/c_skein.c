...
#define SKEIN_T1_BIT(BIT)                                                      \
  ((BIT)-64) /* offset 64 because it's the second word  */

#define SKEIN_T1_POS_TREE_LVL                                                  \
  SKEIN_T1_BIT(112) /* bits 112..118: level in hash tree       */
...
#define SKEIN_T1_POS_FIRST                                                     \
  SKEIN_T1_BIT(126) /* bits 126     : first block flag         */
#define SKEIN_T1_POS_FINAL                                                     \
  SKEIN_T1_BIT(127) /* bit  127     : final block flag         */
...
#define SKEIN_T1_BLK_TYPE_PERS                                                 \
  SKEIN_T1_BLK_TYPE(PERS) /* personalization string */
#define SKEIN_T1_BLK_TYPE_PK                                                   \
  SKEIN_T1_BLK_TYPE(PK) /* public key (for digital signature hashing) */
#define SKEIN_T1_BLK_TYPE_KDF                                                  \
  SKEIN_T1_BLK_TYPE(KDF) /* key identifier for KDF */
...
#define SKEIN_CFG_TREE_LEAF_SIZE_MSK                                           \
  (((u64b_t)0xFF) << SKEIN_CFG_TREE_LEAF_SIZE_POS)
#define SKEIN_CFG_TREE_NODE_SIZE_MSK                                           \
  (((u64b_t)0xFF) << SKEIN_CFG_TREE_NODE_SIZE_POS)
#define SKEIN_CFG_TREE_MAX_LEVEL_MSK                                           \
  (((u64b_t)0xFF) << SKEIN_CFG_TREE_MAX_LEVEL_POS)
...
#define SKEIN_CFG_TREE_INFO_SEQUENTIAL                                         \
  SKEIN_CFG_TREE_INFO(                                                         \
      0, 0,                                                                    \
      0) /* use as treeInfo in InitExt() call for sequential processing */
...
#define Skein_Start_New_Type(ctxPtr, BLK_TYPE)                                 \
  {                                                                            \
    Skein_Set_T0_T1(ctxPtr, 0,                                                 \
                    SKEIN_T1_FLAG_FIRST | SKEIN_T1_BLK_TYPE_##BLK_TYPE);       \
    (ctxPtr)->h.bCnt = 0;                                                      \
  }
...
#define Skein_Show_Key(bits, ctx, key, keyBytes)
                           ...
                           context)? */
#define Skein_Assert(                                                          \
    x, retCode) /* default: ignore all Asserts, for performance */
...
#define SKEIN_256_ROUNDS_TOTAL                                                 \
  (72) /* number of rounds for the different block sizes */
...
const u64b_t SKEIN_512_IV_160[] = {
    MK_64(0x28B81A2A, 0xE013BD91), MK_64(0xC2F11668, 0xB5BDF78F),
...
const u64b_t SKEIN_512_IV_224[] = {
    MK_64(0xCCD06162, 0x48677224), MK_64(0xCBA65CF3, 0xA92339EF),
...
const u64b_t SKEIN_512_IV_256[] = {
    MK_64(0xCCD044A1, 0x2FDB3E13), MK_64(0xE8359030, 0x1A79A9EB),
...
const u64b_t SKEIN_512_IV_384[] = {
    MK_64(0xA3F6C6BF, 0x3A75EF5F), MK_64(0xB0FEF9CC, 0xFD84FAA4),
...
const u64b_t SKEIN_512_IV_512[] = {
    MK_64(0x4903ADFF, 0x749C51CE), MK_64(0x0D95DE39, 0x9746DF03),
...
const u64b_t SKEIN1024_IV_384[] = {
    MK_64(0x5102B6B8, 0xC1894A35), MK_64(0xFEEBC9E3, 0xFE8AF11A),
...
const u64b_t SKEIN1024_IV_512[] = {
    MK_64(0xCAEC0E5D, 0x7C1B1B18), MK_64(0xA01B0E04, 0x5F03E802),
...
const u64b_t SKEIN1024_IV_1024[] = {
    MK_64(0xD593DA07, 0x41E72355), MK_64(0x15B5E511, 0xAC73E00C),
...
static void Skein_256_Process_Block(Skein_256_Ctxt_t *ctx, const u08b_t *blkPtr,
                                    size_t blkCnt,
                                    size_t byteCntAdd) { /* do it in C */
  enum { WCNT = SKEIN_256_STATE_WORDS };
#undef RCNT
...
#error "Invalid SKEIN_UNROLL_256"               /* sanity check on unroll count */
  ...
  u64b_t
      kw[WCNT + 4 +
         RCNT * 2]; /* key schedule words : chaining vars + tweak + "rotation"*/
#else
...
#ifdef SKEIN_DEBUG
  const u64b_t
      *Xptr[4]; /* use for debugging (help compiler put Xn in registers) */
  Xptr[0] = &X0;
  Xptr[1] = &X1;
  Xptr[2] = &X2;
  Xptr[3] = &X3;
  ...
  do {
    ...
    Skein_Get64_LSB_First(w, blkPtr,
                          WCNT); /* get input block in little-endian format */
    DebugSaveTweak(ctx);
...
#ifdef SKEIN_DEBUG
  const u64b_t
      *Xptr[8]; /* use for debugging (help compiler put Xn in registers) */
  Xptr[0] = &X0;
  Xptr[1] = &X1;
  Xptr[2] = &X2;
  Xptr[3] = &X3;
...
static void Skein1024_Process_Block(
    Skein1024_Ctxt_t *ctx, const u08b_t *blkPtr, size_t blkCnt,
    size_t byteCntAdd) { /* do it in C, always looping (unrolled is bigger AND
                            slower!) */
  enum { WCNT = SKEIN1024_STATE_WORDS };
#undef RCNT
...
#error "Invalid SKEIN_UNROLL_1024"              /* sanity check on unroll count */
  ...
  u64b_t X00, X01, X02, X03, X04, X05, X06,
      X07, /* local copy of vars, for speed */
      X08, X09, X10, X11, X12, X13, X14, X15;
  u64b_t w[WCNT]; /* local copy of input block */
#ifdef SKEIN_DEBUG
  const u64b_t
      *Xptr[16]; /* use for debugging (help compiler put Xn in registers) */
  Xptr[0] = &X00;
  Xptr[1] = &X01;
  Xptr[2] = &X02;
  Xptr[3] = &X03;
  ...
  do {
    ...
    ks[14] = ctx->X[14];
    ks[15] = ctx->X[15];
    ks[16] = ks[0] ^ ks[1] ^ ks[2] ^ ks[3] ^ ks[4] ^ ks[5] ^ ks[6] ^ ks[7] ^
             ks[8] ^ ks[9] ^ ks[10] ^ ks[11] ^ ks[12] ^ ks[13] ^ ks[14] ^
             ks[15] ^ SKEIN_KS_PARITY;
...
#define R1024(p0, p1, p2, p3, p4, p5, p6, p7, p8, p9, pA, pB, pC, pD, pE, pF,  \
              ROT, rn)                                                         \
  Round1024(p0, p1, p2, p3, p4, p5, p6, p7, p8, p9, pA, pB, pC, pD, pE, pF,    \
            ROT, rn) Skein_Show_R_Ptr(BLK_BITS, &ctx->h, rn, Xptr);
...
static int Skein_256_Update(Skein_256_Ctxt_t *ctx, const u08b_t *msg,
  ...
  Skein_Assert(ctx->h.bCnt <= SKEIN_256_BLOCK_BYTES,
               SKEIN_FAIL); /* catch uninitialized context */
  ...
  if (msgByteCnt + ctx->h.bCnt > SKEIN_256_BLOCK_BYTES) {
    ...
    if (msgByteCnt > SKEIN_256_BLOCK_BYTES) {
      n = (msgByteCnt - 1) /
          SKEIN_256_BLOCK_BYTES; /* number of full blocks to process */
...
static int Skein_256_Final(Skein_256_Ctxt_t *ctx, u08b_t *hashVal) {
  ...
  Skein_256_Process_Block(ctx, ctx->b, 1,
                          ctx->h.bCnt); /* process the final block */
  ...
  memset(ctx->b, 0,
         sizeof(ctx->b));       /* zero out b[], so it can hold the counter */
  memcpy(X, ctx->X, sizeof(X)); /* keep a local copy of counter mode "key" */
  for (i = 0; i < byteCnt; i += SKEIN_256_BLOCK_BYTES) {
    ((u64b_t *)ctx->b)[0] =
        Skein_Swap64((u64b_t)i); /* build the counter block */
    Skein_Start_New_Type(ctx, OUT_FINAL);
    Skein_256_Process_Block(ctx, ctx->b, 1,
                            sizeof(u64b_t)); /* run "counter mode" */
    n = byteCnt - i; /* number of output bytes left to go */
    if (n >= SKEIN_256_BLOCK_BYTES)
      n = SKEIN_256_BLOCK_BYTES;
    Skein_Put64_LSB_First(hashVal + i, ctx->X,
                          n); /* "output" the ctr mode bytes */
    Skein_Show_Final(256, &ctx->h, n, hashVal + i * SKEIN_256_BLOCK_BYTES);
    memcpy(ctx->X, X,
           sizeof(X)); /* restore the counter mode key for next time */
  }
  return SKEIN_SUCCESS;
}
...
static int Skein_512_Init(Skein_512_Ctxt_t *ctx, size_t hashBitLen) {
  ...
  default:
    ...
    cfg.w[2] = Skein_Swap64(SKEIN_CFG_TREE_INFO_SEQUENTIAL);
    memset(&cfg.w[3], 0,
           sizeof(cfg) - 3 * sizeof(cfg.w[0])); /* zero pad config block */
...
static int Skein_512_Update(Skein_512_Ctxt_t *ctx, const u08b_t *msg,
  ...
  if (msgByteCnt + ctx->h.bCnt > SKEIN_512_BLOCK_BYTES) {
    ...
    if (msgByteCnt > SKEIN_512_BLOCK_BYTES) {
      n = (msgByteCnt - 1) /
          SKEIN_512_BLOCK_BYTES; /* number of full blocks to process */
...
static int Skein_512_Final(Skein_512_Ctxt_t *ctx, u08b_t *hashVal) {
  ...
  for (i = 0; i * SKEIN_512_BLOCK_BYTES < byteCnt; i++) {
    ...
    Skein_512_Process_Block(ctx, ctx->b, 1,
                            sizeof(u64b_t)); /* run "counter mode" */
    n = byteCnt -
        i * SKEIN_512_BLOCK_BYTES; /* number of output bytes left to go */
...
static int Skein1024_Update(Skein1024_Ctxt_t *ctx, const u08b_t *msg,
  ...
  if (msgByteCnt + ctx->h.bCnt > SKEIN1024_BLOCK_BYTES) {
    ...
    if (msgByteCnt > SKEIN1024_BLOCK_BYTES) {
      n = (msgByteCnt - 1) /
          SKEIN1024_BLOCK_BYTES; /* number of full blocks to process */
...
static int Skein1024_Final(Skein1024_Ctxt_t *ctx, u08b_t *hashVal) {
  ...
  for (i = 0; i * SKEIN1024_BLOCK_BYTES < byteCnt; i++) {
    ...
    Skein1024_Process_Block(ctx, ctx->b, 1,
                            sizeof(u64b_t)); /* run "counter mode" */
    n = byteCnt -
        i * SKEIN1024_BLOCK_BYTES; /* number of output bytes left to go */
...
static SkeinHashReturn Update(hashState *state, const SkeinBitSequence *data,
                              SkeinDataLength databitlen);
...
static SkeinHashReturn Update(hashState *state, const SkeinBitSequence *data,
                              SkeinDataLength databitlen) {
  /* only the final Update() call is allowed do partial bytes, else assert an
   * error */
  Skein_Assert((state->u.h.T[1] & SKEIN_T1_FLAG_BIT_PAD) == 0 ||
                   databitlen == 0,
               SKEIN_FAIL);

  Skein_Assert(state->statebits % 256 == 0 && (state->statebits - 256) < 1024,
               SKEIN_FAIL);
  if ((databitlen & 7) == 0) /* partial bytes? */
  {
    ...
    size_t bCnt =
        (databitlen >> 3) + 1; /* number of bytes to handle (nonzero here!) */
    u08b_t b, mask;

    mask = (u08b_t)(1u << (7 - (databitlen & 7))); /* partial byte bit mask */
    b = (u08b_t)((data[bCnt - 1] & (0 - mask)) |
                 mask); /* apply bit padding on final byte */

    switch ((state->statebits >> 8) & 3) {
    case 2:
      Skein_512_Update(&state->u.ctx_512, data,
                       bCnt - 1); /* process all but the final byte    */
      Skein_512_Update(&state->u.ctx_512, &b,
                       1); /* process the (masked) partial byte */
      break;
    case 1:
      Skein_256_Update(&state->u.ctx_256, data,
                       bCnt - 1); /* process all but the final byte    */
      Skein_256_Update(&state->u.ctx_256, &b,
                       1); /* process the (masked) partial byte */
      break;
    case 0:
      Skein1024_Update(&state->u.ctx1024, data,
                       bCnt - 1); /* process all but the final byte    */
      Skein1024_Update(&state->u.ctx1024, &b,
                       1); /* process the (masked) partial byte */
      break;
    default:
      return SKEIN_FAIL;
...
SkeinHashReturn skein_hash(int hashbitlen,
                           const SkeinBitSequence *data, /* all-in-one call */
                           SkeinDataLength databitlen,
                           SkeinBitSequence *hashval) {
  hashState state;
