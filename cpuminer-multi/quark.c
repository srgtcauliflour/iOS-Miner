...
static void quarkhash(void *state, const void *input) {
  ...
  // these uint512 in the c++ source of the client are backed by an array of
  // uint32
  uint32_t hashA[16], hashB[16];

  // do one memcopy to get fresh contexts, its faster even with a larger block
  // then issuing 9 memcopies
...
int scanhash_quark(int thr_id, uint32_t *pdata, const uint32_t *ptarget,
  ...
  // char testdata[] =
  // {""};

  // we need bigendian data...
  // lessons learned: do NOT endianchange directly in pdata, this will all
  // proof-of-works be considered as stale from minerd....
  int kk = 0;
  for (; kk < 32; kk++) {
  ...
  //    if (opt_debug)
  //    {
  //        applog(LOG_DEBUG, "Thr: %02d, firstN: %08x, maxN: %08x, ToDo: %d",
  //        thr_id, first_nonce, max_nonce, max_nonce-first_nonce);
  //    }

  /* I'm to lazy to put the loop in an inline function... so dirty
   * copy'n'paste.... */
  /* i know that i could set a variable, but i don't know how the compiler will
   * optimize it, not that then the cpu needs to load the value *everytime* in a
   * register */
  if (ptarget[7] == 0) {
    do {
      pdata[19] = ++n;
