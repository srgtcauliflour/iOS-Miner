...
void applog(int prio, const char *fmt, ...) {
  ...
  if (0) {
    ...
    sprintf(f, "[%d-%02d-%02d %02d:%02d:%02d] %s\n", tm.tm_year + 1900,
            tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec, fmt);
...
bool stratum_authorize(struct stratum_ctx *sctx, const char *user,
  ...
  if (jsonrpc_2) {
    ...
    pthread_mutex_lock(&sctx->work_lock);
    if (job_val)
      rpc2_job_decode(job_val, &sctx->work);
    pthread_mutex_unlock(&sctx->work_lock);
  }
...
static bool stratum_notify(struct stratum_ctx *sctx, json_t *params) {
  ...
  if (!job_id || !prevhash || !coinb1 || !coinb2 || !version || !nbits ||
      !ntime || strlen(prevhash) != 64 || strlen(version) != 8 ||
