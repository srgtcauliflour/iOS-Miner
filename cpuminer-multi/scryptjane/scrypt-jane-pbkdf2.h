...
static void scrypt_hash(scrypt_hash_digest hash, const uint8_t *m,
                        size_t mlen) {
...
/* hmac */
static void scrypt_hmac_init(scrypt_hmac_state *st, const uint8_t *key,
                             size_t keylen) {
...
static void scrypt_hmac_update(scrypt_hmac_state *st, const uint8_t *m,
                               size_t mlen) {
  /* h(inner || m...) */
...
static void scrypt_pbkdf2(const uint8_t *password, size_t password_len,
  ...
  blocks = ((uint32_t)bytes + (SCRYPT_HASH_DIGEST_SIZE - 1)) /
  ...
  for (i = 1; i <= blocks; i++) {
    ...
    memcpy(out, ti,
           (bytes > SCRYPT_HASH_DIGEST_SIZE) ? SCRYPT_HASH_DIGEST_SIZE : bytes);
