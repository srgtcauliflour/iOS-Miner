...
typedef void(FASTCALL *scrypt_ROMixfn)(
    scrypt_mix_word_t *X /*[chunkWords]*/,
    scrypt_mix_word_t *Y /*[chunkWords]*/,
    scrypt_mix_word_t *V /*[chunkWords * N]*/, uint32_t N, uint32_t r);
#endif

/* romix pre/post nop function */
static void STDCALL scrypt_romix_nop(scrypt_mix_word_t *blocks,
                                     size_t nblocks) {}

/* romix pre/post endian conversion function */
static void STDCALL scrypt_romix_convert_endian(scrypt_mix_word_t *blocks,
                                                size_t nblocks) {
#if !defined(CPU_LE)
  static const union {
    uint8_t b[2];
...
typedef void(STDCALL *chunkmixfn)(scrypt_mix_word_t *Bout /*[chunkWords]*/,
...
static int scrypt_test_mix_instance(chunkmixfn mixfn, blockfixfn prefn,
                                    blockfixfn postfn,
  ...
  for (i = 0; i < 16; i += sizeof(scrypt_mix_word_t)) {
    SCRYPT_WORDTO8_LE(final + i,
                      chunk[1][words - (16 / sizeof(scrypt_mix_word_t)) +
                               (i / sizeof(scrypt_mix_word_t))]);
  }
...
static scrypt_mix_word_t *scrypt_item(scrypt_mix_word_t *base,
                                      scrypt_mix_word_t i,
                                      scrypt_mix_word_t len) {
  return base + (i * len);
}

/* returns a pointer to block i */
static scrypt_mix_word_t *scrypt_block(scrypt_mix_word_t *base,
                                       scrypt_mix_word_t i) {
