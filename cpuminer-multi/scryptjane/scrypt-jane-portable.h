...
#endif
#define COMPILER_GCC                                                           \
  (__GNUC__ * 10000 + __GNUC_MINOR__ * 100 + COMPILER_GCC_PATCHLEVEL)
...
#define CPU_LE
       ...
       (defined(CPU_SPARC) || defined(CPU_PPC) || defined(mc68000) ||          \
        defined(sel)) ||                                                       \
       defined(_MIPSEB))
#define CPU_BE
#else
...
static int scrypt_verify(const uint8_t *x, const uint8_t *y, size_t len) {
...
void scrypt_ensure_zero(void *p, size_t len) {
